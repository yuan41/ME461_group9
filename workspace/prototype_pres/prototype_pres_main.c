...
void main(void)
{
    ...
    EPwm8Regs.TBCTL.bit.CLKDIV = 4;  // changed to 4 to set the clock divide to 16    - yuan41 and jfirst2
    EPwm8Regs.TBCTL.bit.PHSEN = 0;

    EPwm8Regs.TBCTR = 0;   // start the timer at 0  - yuan41 and jfirst2

    EPwm8Regs.TBPRD = 62500;  // changed

    EPwm8Regs.CMPA.bit.CMPA = 0.08 * 62500; // start the position at 0 degrees  - yuan41 and jfirst2
    EPwm8Regs.CMPB.bit.CMPB = 0.08 * 62500; // start the position at 0 degrees  - yuan41 and jfirst2
    ...
    GPIO_SetupPinMux(14, GPIO_MUX_CPU1, 1);  // changed
    GPIO_SetupPinMux(15, GPIO_MUX_CPU1, 1);  // changed
    ...
    ConfigCpuTimer(&CpuTimer1, LAUNCHPAD_CPU_FREQUENCY, 4000); // every 4 milliseconds
    ...
    while(1)
    {
        if (UARTPrint == 1 ) {
            ...
            serial_printf(&SerialA, "Left Wheel: %f   Right Wheel: %f   Target Angle: %f radians \r\n", LeftWheel, RightWheel, targetAngle );
...
void setEPWM8A_RCServo(float angle) {
    ...
    EPwm8Regs.CMPA.bit.CMPA = 625*((angle+90)/22.5 + 4);  // converts an angle in degrees into a CMPA value (between 0 and ??)
